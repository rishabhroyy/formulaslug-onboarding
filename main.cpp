...
int main()
{
   while(true) {
      ...
      is_travel = (abs(pedal_pos_1-pedal_pos_2))/((pedal_pos_1+pedal_pos_2)/2) > 0.1;

      // Ff traveling and timer has not started, start it
      if (is_travel && (timer.elapsed_time().count() == 0)) {
         timer.start();
      
      // If traveling and timer has started and has exceeded 100ms, cut power
      } else if (is_travel && (timer.elapsed_time().count() / 1000) >= 100) {
         printf("Travel happened for >100ms, exiting");
         return 1;

      // If timer is running and we stopped traveling, then stop and reset the timer
      } else if (!is_travel && (timer.elapsed_time().count()) > 0) {
         timer.stop();
         timer.reset();
      }
